namespace Barber.Cli.Commands
{
    ...
    public static class OpenApiCommand
    {
        ...
        {
            var table = new ConsoleTable("#", "Schema Key", "Schema Name", "Property Key", "Property Name", "Type", "Default Value", "Default Value Type", "Matched");
            var count = 0;
            foreach (var item in schemas.OrderBy(e => e.Key))
            {
                count++;
                table.AddRow(count, item.Key, item.Name, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                foreach (var prop in item.Properties)
                {
                    table.AddRow(count, item.Key, item.Name, prop.Key, prop.Name, prop.Type, prop.DefaultValue, prop.DefaultValueType, prop.TypeMatched);
                }
            }
